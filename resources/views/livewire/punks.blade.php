<div class="flex flex-col items-center justify-center px-2 sm:px-4 md:px-6 lg:px-0" x-data="{ modalOpen: @entangle('showAccessoryModal') }">
    ...
    <div class="flex flex-wrap justify-center gap-2 sm:gap-4 mb-4 sm:mb-6">
        <select wire:model.debounce.500ms="color"
                class="border rounded px-2 py-1 sm:px-3 sm:py-2 text-sm sm:text-base text-center">
        ...
        <select wire:model.debounce.500ms="totalAccessories"
                class="border rounded px-2 py-1 sm:px-3 sm:py-2 text-sm sm:text-base text-center">
        ...
        <button @click="modalOpen = true; $wire.openAccessoryModal()"
                :disabled="$wire.openingModal"
            ...
            <template x-if="$wire.selectedAccessories.length > 0">
                <span class="ml-1 text-blue-600 font-semibold">({{ count($selectedAccessories) }})</span>
            </template>
            <svg wire:loading wire:target="openAccessoryModal" class="animate-spin h-4 w-4 text-gray-600"
    ...
    <div x-show="modalOpen" x-transition class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50 px-2">
        <div class="bg-white rounded shadow-lg w-full max-w-2xl max-h-[80vh] flex flex-col overflow-hidden">
            ...
            <div class="flex justify-center space-x-2 px-2 sm:px-4 py-2 border-t bg-white"
                ...
                {{-- Cancel --}}
                <button type="button"
                        wire:click="closeAccessoryModal"
                        class="px-3 py-1 sm:px-4 sm:py-2 bg-gray-200 rounded hover:bg-gray-300 flex items-center justify-center text-sm sm:text-base space-x-2 disabled:opacity-50"
                        wire:loading.class.remove="hover:bg-gray-300"
                        wire:loading.class="pointer-events-none opacity-50">
                    <span>Cancel</span>
                    <svg wire:loading wire:target="closeAccessoryModal" class="animate-spin h-4 w-4 text-gray-600"
                         xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z"></path>
                    </svg>
                ...
                <button type="button"
                        ...
                        class="px-3 py-1 sm:px-4 sm:py-2 bg-yellow-500 text-white rounded hover:bg-yellow-600 flex items-center justify-center text-sm sm:text-base space-x-2 disabled:opacity-50"
                        wire:loading.class.remove="hover:bg-yellow-600"
                        wire:loading.class="pointer-events-none opacity-50">
                    <span>Reset</span>
                    <svg wire:loading wire:target="resetFilters" class="animate-spin h-4 w-4 text-white"
                         xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z"></path>
                    </svg>
                ...
                <button type="button"
                        ...
                        class="px-3 py-1 sm:px-4 sm:py-2 bg-blue-600 text-white rounded hover:bg-blue-700 flex items-center justify-center space-x-2 text-sm sm:text-base disabled:opacity-50"
                    ...
                    <svg wire:loading wire:target="applyFilters" class="animate-spin h-4 w-4 text-white"
                         xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 24">
                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z"></path>
                    </svg>
                </button>
